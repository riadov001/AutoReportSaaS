...
export const services = pgTable("services", {
  ...
  isActive: boolean("is_active").notNull().default(true),
  isVisibleToClients: boolean("is_visible_to_clients").notNull().default(false), // New field
...
// ========== PANEL USERS (AutoReport standalone admin) ==========
...
export const insertPanelUserSchema = createInsertSchema(panelUsers).omit({ id: true, createdAt: true });
...
export const landingSettings = pgTable("landing_settings", {
  ...
  appTagline: text("app_tagline").default("Rapports automobiles intelligents propulsés par l'IA."),
  heroTitle: text("hero_title").default("Diagnostics automobiles nouvelle génération"),
  heroSubtitle: text("hero_subtitle").default("Analysez votre véhicule en quelques secondes grâce à notre moteur d'intelligence artificielle."),
  heroCta: text("hero_cta").default("Analyser mon véhicule"),
  contactEmail: text("contact_email").default("support@autoreport.com"),
  contactPhone: text("contact_phone").default("+33 (0)1 21 40 80 80"),
  contactAddress: text("contact_address").default("75, Rue de la République, 75011 Paris"),
  footerCopyright: text("footer_copyright").default("AutoReport. Tous droits réservés."),
...
// ========== REPAIR SHEETS (Fiches de réparation) ==========

export const repairSheets = pgTable("repair_sheets", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
...
export type RepairSheet = typeof repairSheets.$inferSelect;
export const insertRepairSheetSchema = createInsertSchema(repairSheets).omit({ id: true, createdAt: true, updatedAt: true });
export type InsertRepairSheet = z.infer<typeof insertRepairSheetSchema>;

export type LandingSettings = typeof landingSettings.$inferSelect;
export const insertLandingSettingsSchema = createInsertSchema(landingSettings).omit({ id: true, updatedAt: true });
export type InsertLandingSettings = z.infer<typeof insertLandingSettingsSchema>;
