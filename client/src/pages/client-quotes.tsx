...
import { FileText, Download, Eye, Clock, CheckCircle, CircleX, ExternalLink } from "lucide-react";
...
export default function ClientQuotes() {
  ...
  // Track views when quotes are loaded - mark each as viewed by client
  useEffect(() => {
    if (quotes.length > 0) {
      quotes.forEach(quote => {
        fetch(`/api/quotes/${quote.id}/track-view`, { method: "POST", credentials: "include" }).catch(() => {});
      });
    }
  }, [quotes]);
  ...
  const downloadPDFMutation = useMutation({
    mutationFn: async (quoteId: string) => {
      const res = await apiRequest("GET", `/api/quotes/${quoteId}/pdf`, undefined);
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case "approved":
        return <Badge variant="default" className="gap-1 bg-green-600"><CheckCircle className="h-3 w-3" /> Approuvé</Badge>;
      case "rejected":
        return <Badge variant="destructive" className="gap-1"><CircleX className="h-3 w-3" /> Refusé</Badge>;
      case "completed":
