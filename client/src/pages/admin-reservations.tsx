...
export default function AdminReservations() {
  ...
  useEffect(() => {
    ...
    const prefillDate = params.get("scheduledDate");
  ...
  const filteredReservations = reservations.filter(reservation => {
    ...
    const ref = ((reservation as any).reference ?? "").toLowerCase();
    const matchesSearch = 
  ...
  const deleteReservationMutation = useMutation({
    ...
    onSuccess: () => {
      toast({
        title: "Succès",
        description: "Réservation supprimée définitivement et notification envoyée.",
      });
  ...
  // Correction de l'affichage : s'assurer que les données sont chargées avant le rendu principal
              ...
              <div className="lg:hidden space-y-3">
                {filteredReservations.map((reservation) => (
                  <div
                    key={reservation.id}
                    ref={reservation.id === highlightedId ? highlightedRef : undefined}
                    className={`flex flex-col gap-3 p-3 border rounded-md hover-elevate transition-all duration-500 ${
                      reservation.id === highlightedId 
                        ? "border-primary ring-2 ring-primary/50 bg-primary/5" 
                        : "border-border"
                    }`}
                    data-testid={`reservation-item-mobile-${reservation.id}`}
                  >
                    ...
                    <div className="space-y-1">
                      ...
                      {reservation.scheduledDate && (
                        <p className="text-xs text-muted-foreground">
                          <span className="font-medium">Date:</span> {new Date(reservation.scheduledDate).toLocaleDateString('fr-FR')}
                        </p>
                      )}
                      {reservation.priceExcludingTax && (
                        <p className="text-xs text-muted-foreground">
                          <span className="font-medium">Prix HT:</span> {parseFloat(reservation.priceExcludingTax).toFixed(2)} €
                        </p>
                      )}
                    </div>
                    <div className="flex gap-1 pt-1">
                      <Button
                        ...
                        variant="ghost"
                        onClick={() => openEditDialog(reservation)}
                        data-testid={`button-edit-reservation-mobile-${reservation.id}`}
                      >
                        <Edit className="h-4 w-4" />
                      </Button>
                      <Button
                        ...
                        variant="ghost"
                        onClick={() => openDeleteDialog(reservation)}
                        data-testid={`button-delete-reservation-mobile-${reservation.id}`}
                      >
                        <Trash2 className="h-4 w-4" />
                      </Button>
                    </div>
