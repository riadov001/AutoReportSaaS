...
export default function AdminCalendar() {
  ...
  const generateICS = (reservation: Reservation) => {
    ...
    const icsLines = [
      'BEGIN:VCALENDAR', 'VERSION:2.0', 'PRODID:-//AutoReport//Reservation Calendar//FR',
      'CALSCALE:GREGORIAN', 'METHOD:PUBLISH', 'BEGIN:VEVENT',
      `DTSTART:${formatICSDate(startDate)}`, `DTEND:${formatICSDate(endDate)}`,
      `DTSTAMP:${formatICSDate(new Date())}`, `UID:${reservation.id}@autoreport.fr`,
      `SUMMARY:${serviceName} - ${clientName}`,
      `DESCRIPTION:Client: ${clientName}\\nEmploye: ${employeeName}\\nService: ${serviceName}`,
      'LOCATION:AutoReport Atelier',
  ...
  const generateAllICS = () => {
    ...
    const icsLines = ['BEGIN:VCALENDAR', 'VERSION:2.0', 'PRODID:-//AutoReport//Reservation Calendar//FR',
      'CALSCALE:GREGORIAN', 'METHOD:PUBLISH', ...eventLines, 'END:VCALENDAR'];
    const blob = new Blob([buildICSContent(icsLines)], { type: 'text/calendar;charset=utf-8' });
    const url = URL.createObjectURL(blob);
    const link = document.createElement('a');
    link.href = url;
    link.download = `planning-autoreport-${format(currentDate, 'yyyy-MM')}.ics`;
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    URL.revokeObjectURL(url);
