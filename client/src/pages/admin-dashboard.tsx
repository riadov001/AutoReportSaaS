...
import type { Quote, Invoice, Reservation, User, Service, Garage } from "@shared/schema";
...
interface GarageStatsData {
  totalGarages: number;
  activeGarages: number;
  totalRevenue: number;
  totalMonthRevenue: number;
  garages: GarageStatItem[];
}
...
export default function AdminDashboard() {
  ...
  const buildQueryString = () => {
    ...
    if (isSuperAdmin && garageId !== "all") params.append("garageId", garageId);
    return params.toString() ? `?${params.toString()}` : "";
  };

  const { data: analytics, isLoading: analyticsLoading, refetch } = useQuery<AnalyticsData>({
    queryKey: ["/api/admin/analytics", startDate, endDate, paymentMethod, serviceId, garageId],
    queryFn: async () => {
  ...
  const { data: garageStats, isLoading: garageStatsLoading } = useQuery<GarageStatsData>({
    queryKey: ["/api/superadmin/garage-stats"],
    enabled: isAuthenticated && isSuperAdmin,
  });
  ...
  return (
    <div className="p-4 md:p-6 space-y-6 max-w-7xl mx-auto pb-20">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            {isSuperAdmin && garageId !== "all" 
              ? `Analyse - ${garages?.find(g => g.id === garageId)?.name || "Garage sélectionné"}`
      ...
      {isSuperAdmin && (
        <div className="space-y-4" data-testid="section-garage-stats">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <Card data-testid="card-total-garages">
              <CardContent className="pt-4 pb-3 px-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="text-2xl font-bold" data-testid="text-total-garages">
                      {garageStatsLoading ? <Skeleton className="h-8 w-8" /> : garageStats?.totalGarages ?? 0}
                    </div>
            ...
            <Card data-testid="card-active-garages">
              <CardContent className="pt-4 pb-3 px-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="text-2xl font-bold" data-testid="text-active-garages">
                      {garageStatsLoading ? <Skeleton className="h-8 w-8" /> : garageStats?.activeGarages ?? 0}
                    </div>
            ...
            <Card data-testid="card-total-all-revenue">
              <CardContent className="pt-4 pb-3 px-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="text-2xl font-bold" data-testid="text-total-all-revenue">
                      {garageStatsLoading ? <Skeleton className="h-8 w-16" /> : `${(garageStats?.totalRevenue ?? 0).toLocaleString('fr-FR')} €`}
                    </div>
            ...
            <Card data-testid="card-total-month-revenue">
              <CardContent className="pt-4 pb-3 px-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="text-2xl font-bold" data-testid="text-total-month-revenue">
                      {garageStatsLoading ? <Skeleton className="h-8 w-16" /> : `${(garageStats?.totalMonthRevenue ?? 0).toLocaleString('fr-FR')} €`}
                    </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {garageStatsLoading ? (
              ...
              garageStats?.garages.map(g => (
                <Card key={g.garageId} className="hover-elevate cursor-pointer" data-testid={`card-garage-stat-${g.garageId}`} onClick={() => { setGarageId(g.garageId); setShowFilters(true); }}>
                  <CardHeader className="pb-2 flex flex-row items-center justify-between gap-2">
                    <div className="flex items-center gap-2">
                      <Building2 className="h-4 w-4 text-primary" />
                      <CardTitle className="text-base">{g.garageName}</CardTitle>
                    </div>
                  ...
                  <CardContent>
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                      <div>
                        <div className="text-lg font-bold text-primary">{g.monthRevenue.toLocaleString('fr-FR')} €</div>
                        <p className="text-xs text-muted-foreground">CA ce mois</p>
                      </div>
                      <div>
                        <div className="text-lg font-bold">{g.totalRevenue.toLocaleString('fr-FR')} €</div>
                        <p className="text-xs text-muted-foreground">CA total</p>
                      </div>
                      <div>
                        <div className="text-sm font-semibold">{g.totalInvoices} <span className="text-muted-foreground font-normal">({g.paidInvoices} payées)</span></div>
                        <p className="text-xs text-muted-foreground">Factures</p>
                      </div>
                      <div>
                        <div className="text-sm font-semibold">{g.totalClients}</div>
                        <p className="text-xs text-muted-foreground">Clients</p>
                      </div>
                    </div>
                    <div className="grid grid-cols-2 gap-3 mt-2">
                      <div>
                        <span className="text-xs text-muted-foreground">{g.totalQuotes} devis</span>
                      </div>
                      <div>
                        <span className="text-xs text-muted-foreground">{g.totalReservations} réservations</span>
                      </div>
                    </div>
                  </CardContent>
                </Card>
              ...
              {isSuperAdmin && (
                <div className="space-y-2 sm:col-span-2 lg:col-span-5">
                  ...
                  <Select value={garageId} onValueChange={setGarageId}>
                    <SelectTrigger data-testid="select-garage-filter">
