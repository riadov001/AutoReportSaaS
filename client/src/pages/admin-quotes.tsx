...
import type { Quote, User, ApplicationSettings, Garage } from "@shared/schema";
...
import { Check, X, FileText, Calendar, Download, Plus, Pencil, Tags, Search, Mail, Loader2, Eye, CircleCheckBig, Eye as ViewIcon, Send, MailX, EyeOff, CircleX, ExternalLink } from "lucide-react";
  ...
  return (
    <div className="p-4 md:p-6 space-y-6 max-w-7xl mx-auto">
                  ...
                  <TableBody>
                    {filteredQuotes.map((quote) => (
                      <TableRow key={quote.id} className="hover-elevate group" data-testid={`admin-quote-item-${quote.id}`}>
                        ...
                        {isSuperAdmin && (
                          <TableCell>
                            {getGarageName(quote.garageId) ? (
                              <Badge variant="secondary" data-testid={`badge-garage-${quote.id}`}>{getGarageName(quote.garageId)}</Badge>
                            ) : (
      ...
      <SendEmailDialog
        ...
        defaultSubject={emailDialogQuote ? `Devis N°${emailDialogQuote.reference || emailDialogQuote.id.slice(0, 8).toUpperCase()} - AUTOREPORT - ${emailDialogQuote.quoteAmount ? parseFloat(emailDialogQuote.quoteAmount).toLocaleString("fr-FR", { style: "currency", currency: "EUR" }) : "0,00 €"}` : ""}
