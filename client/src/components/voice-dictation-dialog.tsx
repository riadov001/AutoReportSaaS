...
export function VoiceDictationDialog({
  ...
  const generateEmailMutation = useMutation({
    mutationFn: async () => {
      throw new Error("La dictée vocale n'est plus disponible. Veuillez rédiger l'email manuellement.");
    },
    onSuccess: (data: any) => {
  ...
  const sendEmailMutation = useMutation({
    mutationFn: async () => {
      return await apiRequest("POST", "/api/voice-dictation/send-email", {
        to: clientEmail,
        subject: `AutoReport - ${documentType === 'quote' ? 'Devis' : 'Facture'} ${documentNumber}`,
        body: generatedEmail,
        documentType,
        documentNumber,
        documentId,
