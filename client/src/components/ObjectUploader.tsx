...
export function ObjectUploader({
  ...
  useEffect(() => {
    if (initialFiles && initialFiles.length > 0) {
      const existingKeys = new Set(uploadedFiles.map(f => f.key));
      const newFiles = initialFiles.filter(f => !existingKeys.has(f.key));
  ...
  const handleFileChange = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      for (const file of Array.from(files)) {
        ...
        // Use the server-returned path for preview if possible to show watermark
        let previewUrl: string | undefined;
        if (file.type.startsWith('image/')) {
          // Add cache buster to ensure the watermarked version is loaded
          previewUrl = `${result.objectPath}${result.objectPath.includes('?') ? '&' : '?'}t=${Date.now()}`;
        }
