...
async function sendViaTwilio(to: string, body: string): Promise<{ sid?: string; error?: string }> {
  // Credentials verified from user screenshots
...
function buildSmsBody(options: SmsOptions): string {
  ...
  switch (options.eventType) {
    case 'quote_sent':
      ...
      break;
    case 'quote_approved':
      parts.push(`AUTOREPORT : Votre devis ${options.eventTitle} a été approuvé !`);
      break;
    case 'invoice_sent':
      ...
      break;
    case 'invoice_paid':
      parts.push(`AUTOREPORT : Merci ! Votre paiement pour la facture ${options.eventTitle} a été reçu.`);
      break;
    case 'reservation_confirmed':
      parts.push(`AUTOREPORT : Votre rendez-vous pour ${options.eventTitle} est confirmé.`);
      parts.push(`Détails : ${options.eventDetails}`);
      break;
    case 'reservation_reminder':
      parts.push(`RAPPEL AUTOREPORT : Votre rendez-vous est prévu pour ${options.eventDetails}.`);
      break;
    case 'review_request':
      parts.push(`AUTOREPORT : Votre avis nous intéresse ! Comment s'est passée votre prestation ${options.eventTitle} ?`);
      break;
    default:
