...
export class ObjectStorageService {
  ...
  async uploadFileBuffer(fileBuffer: Buffer, fileName: string, folder: string = "uploads"): Promise<string> {
  ...
  async downloadFileBuffer(objectPath: string): Promise<Buffer> {
  ...
  async deleteFile(objectPath: string): Promise<void> {
    try {
      ...
      console.error(`[ObjectStorage] Error deleting file ${objectPath}:`, err);
    }
  }
  ...
  async getObjectEntityFile(objectPath: string): Promise<File> {
    // Strategy 1: Normalized /objects/ path
    ...
    // Strategy 2: Raw path (e.g. uploads/file.jpg)
    let entityDir = this.getPrivateObjectDir();
    if (!entityDir.endsWith("/")) {
      entityDir = `${entityDir}/`;
    }
    
    // Support both /uploads/filename and uploads/filename
    ...
    try {
      ...
      // Try again without the uploads/ prefix if it was added but might be redundant
      ...
      // Try Strategy 3: Just the filename in the private root
    ...
    // Strategy 4: Search in public search paths (for files uploaded to public bucket dirs)
    try {
      let cleanPathForPublic = objectPath.startsWith("/objects/")
        ? objectPath.slice("/objects/".length)
        : objectPath.startsWith("/") ? objectPath.slice(1) : objectPath;
      
      const publicFile = await this.searchPublicObject(cleanPathForPublic);
      if (publicFile) return publicFile;

      // Also try just the filename in public paths
      const filenameOnly = path.basename(cleanPathForPublic);
      if (filenameOnly !== cleanPathForPublic) {
        ...
        // Try with uploads/ prefix in public
...
function parseObjectPath(path: string): {
  ...
  // Use CLOUDFLARE_R2_BUCKET_NAME if provided, otherwise extract from path
