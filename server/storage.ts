import {
  ...
  panelUsers,
  type PanelUser,
  type InsertPanelUser,
  landingSettings,
  type LandingSettings,
  type InsertLandingSettings,
  repairSheets,
  type RepairSheet,
...
export interface IStorage {
  ...
  getServices(garageId?: string): Promise<Service[]>;
  getAllServices(garageId?: string): Promise<Service[]>;
  ...
  // Accounting Entries
  getAccountingEntries(garageId?: string, filters?: { journal?: string; startDate?: Date; endDate?: Date }): Promise<AccountingEntry[]>;
  ...
  // AI Reports
  getAiReports(userId?: string, garageId?: string): Promise<AiReport[]>;
  getAiReport(id: string): Promise<AiReport | undefined>;
  ...
  getPanelUserById(id: string): Promise<PanelUser | undefined>;
  createPanelUser(data: InsertPanelUser): Promise<PanelUser>;
  countPanelUsers(): Promise<number>;
  getAllPanelUsers(): Promise<PanelUser[]>;
  updatePanelUser(id: string, data: Partial<InsertPanelUser>): Promise<PanelUser>;
  deletePanelUser(id: string): Promise<void>;

  // Repair Sheets
  getAllRepairSheets(): Promise<RepairSheet[]>;
  getRepairSheet(id: string): Promise<RepairSheet | undefined>;
  createRepairSheet(data: InsertRepairSheet): Promise<RepairSheet>;
  updateRepairSheet(id: string, data: Partial<InsertRepairSheet>): Promise<RepairSheet>;
  deleteRepairSheet(id: string): Promise<void>;
}
...
export class DatabaseStorage implements IStorage {
  ...
  async getAllServices(garageId?: string): Promise<Service[]> {
    const conditions = [];
    if (garageId) {
      conditions.push(eq(services.garageId, garageId));
    }
  ...
  async createInvoice(invoiceData: InsertInvoice): Promise<Invoice> {
    ...
    if (existing) {
      // If it exists, we skip it and try again recursively or just increment until free
      // For now, let's just use the max logic as a fallback if counter fails
      const startOfMonth = new Date(now.getFullYear(), now.getMonth(), 1);
  ...
  async getAccountingEntries(garageId?: string, filters?: { journal?: string; startDate?: Date; endDate?: Date }): Promise<AccountingEntry[]> {
  ...
  async getNextEntryNumber(year: number): Promise<string> {
    const count = await db.select({ count: sql<number>`count(*)::int` }).from(accountingEntries)
      .where(sql`EXTRACT(YEAR FROM ${accountingEntries.date}) = ${year}`);
  ...
  // AI Reports
  async getAiReports(userId?: string, garageId?: string): Promise<AiReport[]> {
    const conditions = [];
  ...
  async updateAiReport(id: string, data: Partial<InsertAiReport>): Promise<AiReport> {
  ...
  async updateLandingSettings(data: Partial<InsertLandingSettings>): Promise<LandingSettings> {
    const existing = await this.getLandingSettings();
    if (!existing) {
      const [created] = await db.insert(landingSettings).values({ id: 1, ...data, updatedAt: new Date() }).returning();
      return created;
    }
    const [updated] = await db.update(landingSettings).set({ ...data, updatedAt: new Date() }).where(eq(landingSettings.id, 1)).returning();
    return updated;
  }

  async getPanelUserByEmail(email: string): Promise<PanelUser | undefined> {
    const [user] = await db.select().from(panelUsers).where(eq(panelUsers.email, email.toLowerCase()));
    return user;
  }

  async getPanelUserById(id: string): Promise<PanelUser | undefined> {
    const [user] = await db.select().from(panelUsers).where(eq(panelUsers.id, id));
    return user;
  }

  async createPanelUser(data: InsertPanelUser): Promise<PanelUser> {
    const [user] = await db.insert(panelUsers).values({ ...data, email: data.email.toLowerCase() }).returning();
    return user;
  }

  async countPanelUsers(): Promise<number> {
    const result = await db.select({ count: sql<number>`count(*)` }).from(panelUsers);
    return Number(result[0]?.count || 0);
  }

  async getAllPanelUsers(): Promise<PanelUser[]> {
    return await db.select().from(panelUsers).orderBy(desc(panelUsers.createdAt));
  }

  async updatePanelUser(id: string, data: Partial<InsertPanelUser>): Promise<PanelUser> {
    const [user] = await db.update(panelUsers).set(data).where(eq(panelUsers.id, id)).returning();
    return user;
  }

  async deletePanelUser(id: string): Promise<void> {
    await db.delete(panelUsers).where(eq(panelUsers.id, id));
  }

  // ===== REPAIR SHEETS =====
  async getAllRepairSheets(): Promise<RepairSheet[]> {
    return await db.select().from(repairSheets).orderBy(desc(repairSheets.createdAt));
  }

  async getRepairSheet(id: string): Promise<RepairSheet | undefined> {
    const [sheet] = await db.select().from(repairSheets).where(eq(repairSheets.id, id));
    return sheet;
  }

  async createRepairSheet(data: InsertRepairSheet): Promise<RepairSheet> {
    const [sheet] = await db.insert(repairSheets).values(data).returning();
    return sheet;
  }

  async updateRepairSheet(id: string, data: Partial<InsertRepairSheet>): Promise<RepairSheet> {
    const [sheet] = await db.update(repairSheets)
      .set({ ...data, updatedAt: new Date() })
      .where(eq(repairSheets.id, id))
      .returning();
    return sheet;
  }
